...
>
> ⚠️ **本文件已降级为过程档**：其中的**规范条款**已迁入唯一入口 **`docs/uiux/`**
> （宪章 / 原则 / 令牌表(生成) / 组件七态 / 布局响应式 / 微交互 / 无障碍 / 例外登记 / 规范↔门禁对照）。
> 轮次记录见 `docs/uiux/log/2026-09-13-探索页改版与微交互立规.md`。本文件保留原文，作为决策过程的证据链。

---

## 0. 方向裁决（用户已批：保留视觉语言 / 巨字分档 / P0→P4 顺序）
...
- **四个"0 宽网格轨道"是我自己的误报**（P3-b 查证撤回）：`explore-labels` / `archive-grid` /
  `dv-disclose-grid` / `uw-types` 都是 `repeat(auto-fit, minmax(...))` 在**项目数少于轨道数**时
  折叠空轨道 —— 正常行为，不是"写死几列凑数"。
- **论坛 chip+tab 混排不是混乱**（P2-c 查证撤回）：forum.css 注释写明是 C4/t6-M5-U4 的刻意裁决
  （两组不同性质单选用形态分域 + 2px 竖分隔，避免"同行双份 .tab 双活态抢眼"）。
...
| 提交 | 内容 | 关键实测 |
|---|---|---|
| `b65113d` | P2-a 空/错误/不存在三态分离 | 拦掉接口后 6 处显示「…加载失败（不是「没有…」）」+ 重试；**护栏测试抓出审计没列到的第 5、6 处** |
| `0fbb332` | P2-c 搜索框宽度收进一处 + tab/filter 容器盒模型统一 | 5 处内联 style 清零；demos 560 / models·tasks·forum 320 / 后台用户 260 逐一不变 |
| `6fcd554` | P2-d `useLoadMore` + `<LoadMore>` | **修掉 3 处静默截断 50**（标签/用户/公开用户页第 51 件起消失）；/user/alice 实测「已全部加载（2 件）」 |
| `24e2a98` | P2-e 瀑布流与统计件收敛 | 3 处 CSS 多列 → MasonryGrid（/demo 2 块 6 项、/user 1 块 4 列）；`.tag-stat` 16 处并入 `.mini-stat`，页头高 179/179/179/171 不变 |
| `f4aa292` | P3-a 页宽三档 | /demos 1440→1393、1920→1440；详情页仍 1280；关于正文 640→760；设置两卡对齐 760 |
| `0882693` | P3-b 逐页修正 | 面包屑左缘 89→73（与页头对齐）；首页侧栏去掉第二个滚动区；通知小屏改两行 |
| `c586308` | P4-A 后台别名入口 / 编辑 force 出口 / 关注失败不再静默 | 导航出现「模型别名」且面板可达；`@submit.prevent="submit"` 的 force 陷阱由 build 抓出 |
| `P4-B` | 通知列表服务端筛选 + 分页 | 未读口径真的发 `unread_only`（5 条单测钉住）；修掉"最新 50 条以外的未读看不到" |

**两次由 `build` 抓到 `typecheck` 漏掉的错误**（本仓 CI 把 build 单列一道门的价值）：
1. `ref<T[]>([]) as { value: T[] }` 抹掉 ref 标记 → 模板 `items.length` 取不到（TS2339）。
2. `submit(force = false)` 后模板仍是 `@submit.prevent="submit"` → **submit 事件被当成 force**
   （永远 truthy，编辑一律强推）。已改 `submit()`。
...
| 项 | 状态 | 为什么 |
|---|---|---|
| 通知之外的功能缺口（下载失败提示、评论数刷新、后台队列徽章刷新、公告挂作品、封禁筛选、unread 之外的半接线） | **未做** | 优先级低于已做的 6 条；每条都已定位到文件与行号，可独立执行 |
| 40 处手写空态/加载态标记的机械替换 | **部分**（P4-B 顺手改了 3 处） | 收益只在整洁度、风险是 40 处 `v-if/v-else` 链改造；按"谁改谁迁"跟着 P3 逐页做 |
| RF-5b i18n 中文单一来源 | **不做** | 三类痛点（矛盾文案/缺词条/死词条）已被 i18n 门禁覆盖 |
| `.route-page` 无样式（20 视图在用、0 处 CSS） | **不改** | 它是纯包裹层、无样式也无害；定义它会一次性影响 20 页 |
| 小触点（<32px）统一到 36px | **未做** | 会改到全站按钮尺寸，属独立视觉决策，混进来会让截图对照失去意义 |
| TaskDetailView `table-layout:fixed` | **降级不做** | 真实数据下**未复现**溢出（逐格测 0 溢出），只在超长内容下有风险 |
| DemosView 剩余拆分（抽屉内容层/无限滚动/分组） | **未做** | 与本次目标（布局+视觉一致性）无关，属 RF-4 的延续 |

---

## 11. 设计稿 → 计划 → 落地：一次真实漂移的教训（模型格 D 变体）

**事实**：探索页模型格最终选定「D 变体 = 半嵌入图标块 + 厂商底色 + **顶带 + 左带**」。
我先出了视觉稿（稿子里 D **明确是顶带 + 左带**，带子用负偏移压住卡片边框），
但写实施计划时把 D 描述成「底色 + **左缘**细带」（**漏了顶带**），实现又照着计划走 ——
于是**用户按稿子批准的东西，落地时少了一条带子**，是用户看截图时发现的。

第二处更隐蔽：稿子用 `left:-4px`（压住 4px 黑边框），实现用了 `left:0`（落在边框**内侧**）——
两者观感不同（"边框本身被染色" vs "框里多一条色条"）。

**修法与护栏**
- 模板补回顶带；带子改为 `.explore-band--top` / `.explore-band--left` 两类
- 偏移改用 `calc(var(--border-w) * -1)` **而不是照抄稿子的 -4px**：稿子为快出效果写死值，
  落地必须跟令牌走，否则改边框粗细就会错位（这是"稿子能看、落地要改"的**正当差异**，已在代码注释里写明）
- 新增 2 条护栏：① D 变体必须**同时**有顶带与左带（禁止"光秃秃一条"的写法）；② 偏移必须走 `--border-w` 令牌
- 破环验证：删掉顶带 → 测试报红

**规则（以后照做）**
1. **存在设计稿时，实施计划必须逐字引用稿子的规格**（尤其"有几条带子/几个元素"这类可数量化的部分），
   不许用概括语言重述 —— 概括正是丢细节的地方。
2. **稿子里的死值（-4px 这类）在落地时要改用令牌表达**，并在注释里说明为何与稿子不同。
3. 视觉稿的"取舍一句话"要连**代价**一起写（D 是 4 个变体里颜色负载最大的），并预留返工出口
   （例如"退到 C 变体只需删掉图标块背景色一行"）。

---

## 12. 题目行接真实封面 + 200px 缩略图管线（已完成）

设计稿 §3「题目行：接真实 demo 封面」在上一轮就已批准并写死了规格；本轮把它落地。
**规格六条逐条对应实现与验证**（§11 规则 1：不许用概括语言重述）：

| 稿子规格 | 实现 | 验证 |
|---|---|---|
| 72×48、2px 黑边、`object-fit:cover`、`flex:0 0 auto` | `.task-line-cover`（`styles/pages/models-tasks.css`） | 护栏断言四条 CSS 声明逐字存在 |
| `width`/`height` 显式声明防 CLS | `<img width="72" height="48">` | 护栏断言两个属性 |
| `loading="lazy"` + `decoding="async"` + `alt=""` | 同元素三个属性 | 护栏断言 |
| 无封面 → 不渲染、不留空洞 | `v-if="coverOk(tk)"`（`coverOk` = 有 URL 且未加载失败） | 护栏断言 `v-if` 与 `.has-cover` **同一个判定** |
| 走 200px 缩略图 | `storage.make_cover_thumb()`（最长边 200 / WebP q80，只缩不放） | 单测：尺寸、幂等（同输入同字节） |
| 预算 8 张 ≈ 120KB | 实测 **27.3KB**（原图 580KB，−95%） | `performance` 实测网络字节 + 缩略图单测 |

**后端（字段与取数）**
- `Demo.cover_thumb_url`（`_ensure_demo_columns` 增量列，首次加列前自动备份）+ `save_cover()` 返回 `(封面, 缩略图)` 二元组：
  两个文件必须一起写，单值返回时漏写只会静默少图（无异常、无日志）。
- `storage.cover_thumb_url()` 是命名规则的**唯一定义处**（`x.webp → x-thumb.webp`；SVG / 非 webp / 外链 → `""`），
  写入路径与回填脚本共用；规则幂等（重复套用不叠后缀）。
- 代表封面 `task_service.representative_covers()`：该题下**排序第一且有缩略图**的已上架作品，
  排序口径与 `task_chain()` **完全一致**（否则"列表封面是 A、点进去链条第一行是 B"）。
  没有缩略图的作品直接跳过 —— **宁可不放图，也不拿 `default.svg` 冒充作品封面**。
- 接口：`TaskSummaryOut.cover_thumb_url`（默认 `""`），`/explore` 与 `/tasks` 同时受益。

**数据管线**：`scripts/backfill_cover_thumbs.py`（幂等、OSS best-effort、容器内 `python - < 脚本`）。
顺序纪律：**先上线后端（列自愈）→ 再跑回填**；脚本没跑之前列表照旧无图，不会坏。

**实测（生产规模镜像：8 道真实题 / 74 件作品 / 真实封面）**

| 视口 | 行高 前→后 | 行宽 | 缩略图 | 页面高 前→后 | 横向溢出 |
|---|---|---|---|---|---|
| 1440 | 83 → 105（+22） | 456 | 8/8 · 27.3KB | 1636 → 1702 | 0 |
| 1024 | 83 → 105（+22） | 483 | 8/8 · 27.3KB | 1916 → 2004 | 0 |
| 720 | — → 101 | 673 | 8/8 · 27.3KB | — → 2798 | 0 |
| 390 | 81 → 103~107（+22~26） | 358 | 8/8 · 27.3KB | 3191 → 3387（+6.1%） | 0 |

**移动端那一处返工（值得单独记）**：缩略图进来后移动端行高 81 → 132（+51，整页 +321px），
而且 8 行高低不齐。原因是`.task-line-desc` 的 `flex: 1 0 100%` 会让摘要独占一行，
72px 图占位后 CTA 被挤到第三行。改成 `flex: 1 1 60px`（**假设尺寸**变小 → CTA 与摘要同行，
摘要仍长到 237px）后回到 103~107。
**试过但否决的写法**：`flex-basis: 0` —— 短标题的行会把摘要压成 **3px**，等于静默删掉题面摘要，
而"一行题面摘要"正是这个页面让读者判断要不要点进去的依据（护栏里写死了不许用 `1 1 0`）。

**像素一致性（本轮验收证据）**：静态快照 vs 实时页截图

| 视口 | 结论 |
|---|---|
| 1024 / 720 / 390 | **SHA-256 完全相同** |
| 1440 | 180 个孤立抗锯齿像素（占 0.0073%，最大连通块 7px，全在模型格图标边缘）；**题目段 0 像素差异** |

1440 那 180 个像素不是布局差异：同一实时页连拍两次**逐字节相同**（排除截图随机性），
且分区域统计显示差异全在未改动的模型格。另外做了**结构证明**：把封面 `<img>`、`has-cover`
类与 `v-if` 占位注释规范化掉之后，改动前后两份快照**逐字节相同（380,057B）**——
这一轮确实只动了封面这一处。

**明确不做**：`/tasks` 列表页与 `DemoView`/`ModelDetailView` 的题目行不上图（它们是另一套结构）；
mock 模式不造封面（无媒体服务，图必然 404）；`DemoSummaryOut` 不暴露缩略图；
历史非 webp 封面不迁移（`cover_thumb_url` 返回空即可）。
