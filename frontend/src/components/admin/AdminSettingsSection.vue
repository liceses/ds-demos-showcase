...
<template>
  <div>
    <div class="card card-mint" style="max-width: 520px; padding: var(--sp-24); margin-bottom: var(--sp-20)">
      <h2 style="margin-bottom: var(--sp-12)">存储</h2>
      ...
      <p class="hint" style="margin-bottom: var(--sp-12)">本地是完整存储（全量文件在服务器），OSS 只是镜像。切换模式：修改服务器 .env 的 <code>OSS_ENABLED</code>（false=本地 / true=OSS）+ <code>docker compose up -d backend</code> 重建生效。</p>
    ...
    <div class="card card-default" style="max-width: 520px; padding: var(--sp-24)">
      <label class="field">
        <input v-model="settings.auto_approve" type="checkbox" style="width: var(--sp-20); height: 20px; margin-right: var(--sp-8); vertical-align: middle" />
        新上传 Demo 自动通过审核（登录用户）
      </label>
      <label class="field">
        <input v-model="settings.auto_approve_public" type="checkbox" style="width: 20px; height: 20px; margin-right: var(--sp-8); vertical-align: middle" />
        未注册（public）上传自动通过审核
      </label>
      <p class="hint" style="margin-bottom: var(--sp-14)">开启「未注册放行」后，任何人（含 AI agent）不注册即可上传并即时上线，建议配合限流与 UPLOAD_CODE 信任通道使用。</p>
      <label class="field">
        <input v-model="settings.fun_mode" type="checkbox" style="width: 20px; height: 20px; margin-right: var(--sp-8); vertical-align: middle" />
        整活模式（astra 灰测作品收集）
      </label>
      <p class="hint" style="margin-bottom: var(--sp-14)">
