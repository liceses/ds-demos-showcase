...
import type { DemoDetail, DerivedTag, TaskDetail, TaskSuggestItem } from '../api/types'
...
function suggestText() {
  return [title.value.trim(), description.value.trim(), prompt.value.trim()].filter(Boolean).join('\n')
}
...
watch([title, description, prompt], () => {
